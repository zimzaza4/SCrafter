...
public class BasicEliteCrafter extends AbstractEnergyCrafter {
    ...
    public static final RecipeType TYPE = new RecipeType(new NamespacedKey(SlimefunZT.getInstance(), "ZIM_WORKBENCH"),
            Items.SoulCrafter, (stacks, stack) -> {
    ...
    public BasicEliteCrafter(Category category, SlimefunItemStack item, RecipeType type, ItemStack[] recipe,
            int energy) {
    ...
    public void setupMenu(@Nonnull BlockMenuPreset blockMenuPreset) {
        ...
        blockMenuPreset.addItem(RECIPE_SLOT, new CustomItem(Material.BOOK, "&c精英锻造&c - I"),
    ...
    public void craft(@Nonnull Block b, @Nonnull BlockMenu inv, @Nonnull Player p) {
        ...
        if (charge < this.energy) { // not enough energy
            p.sendMessage(new String[] { ChatColor.RED + "电量不足了",
