...
public class VoidWorld extends ChunkGenerator {
    ...
    public ChunkData generateChunkData(World world, Random random, int x, int z, BiomeGrid biome) {
        ...
        if (noise==null){
            ...
            noise.setYScale(random.nextDouble()/100);
            noise.setXScale(random.nextDouble()/100);
        }
        if (fognoise==null) {
        ...
        if (landnoise==null) {
            ...
            landnoise.setYScale(random.nextDouble()/100);
            landnoise.setXScale(random.nextDouble()/100);
        }
        if (webnoise==null) {
        ...
        for (int x1 = 0; x1 < 16; x1++) {
            for (int z1 = 0; z1 < 16; z1++) {
                int realX = x * 16 + x1;
                int realZ = z * 16 + z1;
                double noiseValue = noise.noise(realX, realZ, 0.8D, 0.5D);
